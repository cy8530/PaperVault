...
Paper Vault 的 API 花费来自两个 LLM 调用层级。嵌入模型完全本地运行，零费用。

### 双模型分工

| 层级 | 负责任务 | 调用频率 | 典型模型 |
|------|---------|---------|---------|
| **Light** | 元数据提取、RAG judge、章节匹配、论文筛选 | 高频、小请求 | DeepSeek-V4-Flash / GPT-4o-mini |
| **Primary** | 笔记生成、RAG 最终回答 | 低频、大请求 | DeepSeek-V4 / GPT-4o |

### 单篇论文导入花费

以一篇典型 30 页、~15K token 的英文论文为例（按 DeepSeek 官方 API 价格估算）：

| 步骤 | 模型 | 输入 token | 输出 token | 花费 |
|------|------|-----------|-----------|------|
| 元数据提取 | Light | ~2,000 | ~100 | ¥0.0006 |
| 笔记生成 | Primary | ~15,000 | ~3,000 | ¥0.018 |
| 文本嵌入 | 本地 | — | — | ¥0 |
| **合计** | | **~17,000** | **~3,100** | **≈¥0.019** |

> 导入 50 篇论文总花费约 ¥1，100 篇约 ¥2。

如果使用 SiliconFlow 等国产 API 代理商，价格通常更低（DeepSeek-V3 约 ¥1/百万 token 输入）。

### RAG 问答花费

以一次典型提问为例（检索 5 篇论文，自适应深度）：

| 阶段 | 模型 | 输入 token | 输出 token | 花费 |
|------|------|-----------|-----------|------|
| 问题嵌入 | 本地 | — | — | ¥0 |
| 论文筛选 | Light | ~500 | ~50 | ¥0.0001 |
| 深度判断 (judge) | Light | ~2,000 | ~5 | ¥0.0004 |
| 章节匹配 | Light | ~1,000 | ~200 | ¥0.0003 |
| 生成回答 (Level 2) | Primary | ~6,000 | ~1,500 | ¥0.004 |
| **合计** | | **~9,500** | **~1,755** | **≈¥0.005** |

不同深度级别对比：

| 深度 | 场景 | 检索量 | 单次花费 |
|------|------|-------|---------|
| Level 1 (notes) | "这篇论文做了什么" | 仅笔记 | ≈¥0.002 |
| Level 2 (moderate) | "对比方法 A 和方法 B" | ~1/8 原文 | ≈¥0.005 |
| Level 3 (extensive) | "完整推导公式 X" | ~1/3 原文 | ≈¥0.01 |
| all (full text) | 手动触发全文检索 | 全部原文 | ≈¥0.03 |

### 月度花费估算

| 使用强度 | 每月导入 | 每日提问 | 月花费（约） |
|---------|---------|---------|------------|
| 轻度 | 10 篇 | 5 次 | ¥1.5 |
| 中度 | 30 篇 | 20 次 | ¥4.5 |
| 重度 | 100 篇 | 50 次 | ¥12 |
...
- **设置 `LIGHT_MODEL_ID`** — 使用廉价快速模型处理元数据和 judge，比统一用主模型节省 ~60% 的辅助调用花费
- **RAG 优先 Level 1/2** — 自适应判断已覆盖大部分场景，避免手动用 `-d all` 全文检索
- **使用国产 API** — SiliconFlow、DeepSeek 官方的模型价格远低于 OpenAI，兼容 OpenAI 格式无需改代码
- **嵌入免费** — 768 维向量在本地 CPU 上毫秒级完成，不产生任何 API 费用
- **一次导入，永久复用** — 论文导入后笔记和向量索引持久存储，重复提问不会再产生导入花费
